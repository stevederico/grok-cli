...
This page contains tutorials for interacting with Grok CLI.
...
In your project's root directory, create or open the [`.grok-cli/settings.json` file](./configuration.md). Within the file, add the `mcpServers` configuration block, which provides instructions for how to launch the GitHub MCP server.
...
Grok CLI uses this value in the `mcpServers` configuration that you defined in the `settings.json` file.

#### Launch Grok CLI and verify the connection

When you launch Grok CLI, it automatically reads your configuration and launches the GitHub MCP server in the background. You can then use natural language prompts to ask Grok CLI to perform GitHub actions. For example:
