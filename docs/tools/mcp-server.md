# MCP servers with the Grok CLI

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the Grok CLI.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the Grok CLI through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the AI model and your local environment or other services like APIs.

An MCP server enables the Grok CLI to:
...
- **Access resources:** Read data from specific resources (though the Grok CLI primarily focuses on tool execution).

With an MCP server, you can extend the Grok CLI's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.
...
The Grok CLI supports three MCP transport types:
...
The Grok CLI uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.

### Configure the MCP server in settings.json

You can configure MCP servers at the global level in the `~/.grok-cli/settings.json` file or in your project's root directory, create or open the `.grok-cli/settings.json` file. Within the file, add the `mcpServers` configuration block.
...
When the Grok CLI starts, it performs MCP server discovery through the following detailed process:
...
3. **Name sanitization:** Tool names are cleaned to meet LLM API requirements:
...
This comprehensive integration makes MCP servers a powerful way to extend the Grok CLI's capabilities while maintaining security, reliability, and ease of use.
