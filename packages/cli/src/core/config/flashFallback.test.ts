...
import { DEFAULT_GROK_MODEL, DEFAULT_GROK_FLASH_MODEL } from './models.js';
...
describe('Flash Model Fallback Configuration', () => {
  ...
  describe('setModel', () => {
    it('should update the model and mark as switched during session', () => {
      ...
      config.setModel(DEFAULT_GROK_FLASH_MODEL);

      expect(config.getModel()).toBe(DEFAULT_GROK_FLASH_MODEL);
    ...
    it('should handle multiple model switches during session', () => {
      ...
      config.setModel('grok-1.5-pro');
      expect(config.getModel()).toBe('grok-1.5-pro');
      expect(config.isModelSwitchedDuringSession()).toBe(true);
    });
    ...
    it('should only mark as switched if contentGeneratorConfig exists', () => {
      ...
      newConfig.setModel(DEFAULT_GROK_FLASH_MODEL);
