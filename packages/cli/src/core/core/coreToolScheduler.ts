...
// Simple replacement for getResponseTextFromParts
function getResponseTextFromParts(parts: Part[]): string {
...
export class CoreToolScheduler {
  ...
  async schedule(
    ...
    const newToolCalls: ToolCall[] = requestsToProcess.map(
      (reqInfo): ToolCall => {
        const toolInstance = toolRegistry.getTool(reqInfo.name || reqInfo.toolName);
        if (!toolInstance) {
          return {
            status: 'error',
            request: reqInfo,
            response: createErrorResponse(
              reqInfo,
              new Error(`Tool "${reqInfo.name || reqInfo.toolName}" not found in registry.`),
            ),
            durationMs: 0,
          };
        }
      ...
      try {
        if (this.approvalMode === ApprovalMode.YOLO) {
          ...
          const confirmationDetails = await toolInstance.shouldConfirmExecute(
            reqInfo.args || reqInfo.parameters,
            signal,
          );
          ...
          if (confirmationDetails) {
            ...
            const wrappedConfirmationDetails: ToolCallConfirmationDetails = {
              ...confirmationDetails,
              onConfirm: originalOnConfirm ? (outcome: ToolConfirmationOutcome) =>
  ...
  private attemptExecutionOfScheduledCalls(signal: AbortSignal): void {
    ...
    if (allCallsFinalOrScheduled) {
      ...
      callsToExecute.forEach((toolCall) => {
        ...
        const toolName = scheduledCall.request.name || scheduledCall.request.toolName;
        ...
        scheduledCall.tool
          .execute(scheduledCall.request.args || scheduledCall.request.parameters, signal, liveOutputCallback)
