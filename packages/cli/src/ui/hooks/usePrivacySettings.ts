...
import { GaxiosError } from '../../core/__stubs__/gaxios.js';
import { useState, useEffect, useCallback } from 'react';
import { Config, CodeAssistServer, UserTierId } from '../../core/index.js';
...
function getCodeAssistServer(config: Config): CodeAssistServer {
  const server = config.getProviderClient().getContentGenerator();
...
async function getTier(server: CodeAssistServer): Promise<UserTierId> {
  const loadRes = await server.loadCodeAssist(server.projectId);
...
async function setRemoteDataCollectionOptIn(
  ...
  const resp = await server.setCodeAssistGlobalUserSetting('freeTierDataCollectionOptin', optIn);
  return resp.freeTierDataCollectionOptin;
}
