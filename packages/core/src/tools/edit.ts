...
// Removed Google dependencies - edit correction disabled
  ...
  implements ModifiableTool<EditToolParams>
{
  ...
  // Client removed - edit correction disabled
...
**Multiple replacements:** Set \`expected_replacements\` to the number of occurrences you want to replace. The tool will replace ALL occurrences that match \`old_string\` exactly. Ensure the number of replacements matches your expectation.`,
  ...
  private async calculateEdit(
    ...
    if (params.old_string === '' && !fileExists) {
      ...
      // Edit correction disabled - using original params
      finalOldString = params.old_string;
      finalNewString = params.new_string;
      occurrences = currentContent.split(params.old_string).length - 1;
