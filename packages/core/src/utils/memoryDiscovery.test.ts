...
const ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST = DEFAULT_CONTEXT_FILENAME;
...
describe('loadServerHierarchicalMemory', () => {
  ...
  let GLOBAL_GROK_FILE: string; // Defined in beforeEach
  ...
  beforeEach(() => {
    ...
    GLOBAL_GROK_DIR = path.join(USER_HOME, GROKCLI_CONFIG_DIR);
    GLOBAL_GROK_FILE = path.join(
      GLOBAL_GROK_DIR,
  ...
  it('should load ORIGINAL_GROK_MD_FILENAME files by upward traversal from CWD to project root', async () => {
    const projectRootGrokFile = path.join(
      PROJECT_ROOT,
      ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
    );
    const srcGrokFile = path.join(
      CWD,
      ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
    );
    ...
    mockFs.access.mockImplementation(async (p) => {
      if (p === projectRootGrokFile || p === srcGrokFile) {
        return undefined;
      }
      throw new Error('File not found');
    });
    ...
    const expectedContent =
      `--- Context from: ${path.relative(CWD, projectRootGrokFile)} ---\nProject root memory\n--- End of Context from: ${path.relative(CWD, projectRootGrokFile)} ---\n\n` +
      `--- Context from: ${ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST} ---\nSrc directory memory\n--- End of Context from: ${ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST} ---`;
    ...
    expect(mockFs.readFile).toHaveBeenCalledWith(srcGrokFile, 'utf-8');
  });

  it('should load ORIGINAL_GROK_MD_FILENAME files by downward traversal from CWD', async () => {
    const subDir = path.join(CWD, 'subdir');
    const subDirGrokFile = path.join(
      subDir,
      ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
    );
    const cwdGrokFile = path.join(
      CWD,
      ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
    );

    mockFs.access.mockImplementation(async (p) => {
      if (p === cwdGrokFile || p === subDirGrokFile) return undefined;
      throw new Error('File not found');
    });

    mockFs.readFile.mockImplementation(async (p) => {
      if (p === cwdGrokFile) return 'CWD memory';
      if (p === subDirGrokFile) return 'Subdir memory';
      throw new Error('File not found');
    });
    ...
    mockFs.readdir.mockImplementation((async (
      ...
      if (p === CWD) {
        return [
          {
            name: ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
            isFile: () => true,
            isDirectory: () => false,
          } as Dirent,
          {
    ...
    const expectedContent =
      `--- Context from: ${ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST} ---\nCWD memory\n--- End of Context from: ${ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST} ---\n\n` +
      `--- Context from: ${path.join('subdir', ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST)} ---\nSubdir memory\n--- End of Context from: ${path.join('subdir', ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST)} ---`;

    expect(memoryContent).toBe(expectedContent);
    expect(fileCount).toBe(2);
  });

  it('should load and correctly order global, upward, and downward ORIGINAL_GROK_MD_FILENAME files', async () => {
    setGrokMdFilename(ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST); // Explicitly set for this test
    ...
    const projectParentGrokFile = path.join(
      projectParentDir,
      ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
    );
    const projectRootGrokFile = path.join(
    ...
    mockFs.stat.mockImplementation(async (p) => {
      if (p === path.join(PROJECT_ROOT, '.git')) {
        return { isDirectory: () => true } as Stats;
      } else if (p === path.join(PROJECT_ROOT, '.grok-cli')) {
        return { isDirectory: () => true } as Stats;
      }
      throw new Error('File not found');
    });
    ...
    mockFs.access.mockImplementation(async (p) => {
      if (
        ...
        p === projectParentGrokFile ||
        p === projectRootGrokFile ||
        p === cwdGrokFile ||
        p === subDirGrokFile
      ) {
        return undefined;
    ...
    mockFs.readFile.mockImplementation(async (p) => {
      ...
      if (p === projectParentGrokFile) return 'Project parent memory';
      if (p === projectRootGrokFile) return 'Project root memory';
      if (p === cwdGrokFile) return 'CWD memory';
      if (p === subDirGrokFile) return 'Subdir memory';
      throw new Error('File not found');
    });
    ...
    const relPathGlobal = path.relative(CWD, GLOBAL_GROK_FILE);
    const relPathProjectParent = path.relative(CWD, projectParentGrokFile);
    const relPathProjectRoot = path.relative(CWD, projectRootGrokFile);
    const relPathCwd = ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST;
  ...
  it('should ignore specified directories during downward scan', async () => {
    ...
    const ignoredDirGrokFile = path.join(
      ignoredDir,
      ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
    ); // Corrected
    const regularSubDir = path.join(CWD, 'my_code');
    const regularSubDirGrokFile = path.join(
      regularSubDir,
      ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST,
    );

    mockFs.access.mockImplementation(async (p) => {
      if (p === regularSubDirGrokFile) return undefined;
      if (p === ignoredDirGrokFile)
        throw new Error('Should not access ignored file');
      throw new Error('File not found');
    });

    mockFs.readFile.mockImplementation(async (p) => {
      if (p === regularSubDirGrokFile) return 'My code memory';
      throw new Error('File not found');
    });
    ...
    const expectedContent = `--- Context from: ${path.join('my_code', ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST)} ---\nMy code memory\n--- End of Context from: ${path.join('my_code', ORIGINAL_GROK_MD_FILENAME_CONST_FOR_TEST)} ---`;
